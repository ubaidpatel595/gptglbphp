...
function Faculty(){
    return(
        ...
        <div class="flex" style={{margin:"0px"}}>
            <div id="act-btns">
                <button onClick={()=>{show('action',['gen_reports','upload','upload_opts'])}}>Mark Attendance</button><br/>
                <button onClick={()=>{show('gen_reports',['upload','action','upload_opts'])}}>Generate Reports</button>  <br/> 
                <button onClick={()=>{show('upload',['gen_reports','upload_opts'])}}>Upload</button><br/>
            </div>
